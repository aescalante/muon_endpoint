...
'''
		...
		print "size of master tree: ",self.mastertree.GetEntries()
                print "show master tree: ",self.mastertree.Show(2)
                print "show master tree: ",self.mastertree.Print() 
#                print "scan master tree: ",self.mastertree.Scan("k:eta:phi:ptErr")
		self.hasEta = False
		self.hasPhi = False

#                self.brbMu = self.mastertree.GetBranch("trk1")
#                std::vector<Muon> muons;
#                std::vector<Muon>* pmuons = &muons;
                self.mastertree.GetEntry(0);




#                self.brbMu = self.mastertree.GetBranch("trk1")
#                brbMu->SetAddress(&pmuons);

		self.lfeta 	= self.mastertree.GetBranch("trk1").GetLeaf("eta")
                print "lfeta: ", self.lfeta.GetValue() 
